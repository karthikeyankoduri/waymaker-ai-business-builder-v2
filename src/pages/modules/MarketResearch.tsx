        ...
        return (
            <div className="flex flex-col items-center">
                <div className="relative w-full aspect-square max-w-[400px] flex items-center justify-center mb-8">
                    {/* TAM */}
                    <div className="absolute inset-0 rounded-full border border-white/20 bg-white/5 flex flex-col items-start justify-center pl-8 transition-transform hover:scale-105 duration-300">
                        <span className="text-3xl font-serif text-white">{tam?.value}</span>
                        <span className="text-xs font-mono text-white/50 tracking-widest mt-1">TAM</span>
                    </div>
                    {/* SAM */}
                ...
                <div className="w-full text-left bg-black/40 border border-white/5 rounded-xl p-6  space-y-4">
                    <div className="flex flex-col"><span className="text-xs font-mono text-white/40 uppercase">TAM • {tam?.label}</span><span className="text-sm text-white/60">{tam?.description || "Total global market demand."}</span></div>
                    <div className="flex flex-col"><span className="text-xs font-mono text-emerald-500/70 uppercase">SAM • {sam?.label}</span><span className="text-sm text-white/60">{sam?.description || "Segment targeted by these services."}</span></div>
    ...
    return (
        <div className="max-w-7xl mx-auto space-y-6 animate-in fade-in duration-300 pb-20">
            ...
            {!activeProject.marketResearch ? (
                ...
                <div className="glass-card p-8 md:p-12 relative overflow-hidden text-white/70">
                    ...
                    {parsedData.marketAnalysis ? (
                                        ...
                                        {parsedData.marketAnalysis.topDownApproach?.map((item, i, arr) => {
                                            ...
                                            return (
                                                <div key={i} className="flex flex-col items-center w-full">
                                                    ...
                                                    >
                                                        <span className="text-xs font-mono text-white/50 uppercase tracking-wider">{item.label}</span>
                            ...
                            {parsedData.positionInMarket && (
                                <div className="glass-card p-6 md:p-10 border border-white/10">
                                    ...
                                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                                        ...
                                        <div className="relative w-full aspect-square border-4 border-white/10 rounded-2xl flex flex-col bg-aura-dark/40 p-12">
                                            {/* Stark Black Axes */}
                                            <div className="absolute inset-x-8 top-1/2 -translate-y-1/2 h-1.5 bg-white/10 rounded-full" />
                                            <div className="absolute inset-y-8 left-1/2 -translate-x-1/2 w-1.5 bg-white/10 rounded-full" />
                                            
                                            {/* Axis Labels representing the lines */}
                                            <div className="absolute -top-6 left-1/2 -translate-x-1/2 text-xs font-mono text-white/50">{parsedData.positionInMarket.yAxis?.top || 'High Price'}</div>
                                            <div className="absolute -bottom-6 left-1/2 -translate-x-1/2 text-xs font-mono text-white/50">{parsedData.positionInMarket.yAxis?.bottom || 'Low Price'}</div>
                                            <div className="absolute top-1/2 -left-20 -translate-y-1/2 -rotate-90 text-xs font-mono text-white/50">{parsedData.positionInMarket.xAxis?.left || 'Low Quality'}</div>
                                            <div className="absolute top-1/2 -right-20 -translate-y-1/2 rotate-90 text-xs font-mono text-white/50">{parsedData.positionInMarket.xAxis?.right || 'High Quality'}</div>
                                        ...
                                        <div className="flex flex-col items-center justify-end h-full gap-5 pb-12 pt-12 relative px-8">
                                            ...
                                            {parsedData.positionInMarket.pyramid?.map((level, i) => {
                                                ...
                                                const colors = [
                                                    ...
                                                    'bg-gradient-to-b from-emerald-900 to-aura-dark text-emerald-100 border-2 border-emerald-800 shadow-[0_10px_30px_rgba(0,0,0,0.5)] z-10'
                                                ];
                            ...
                            {parsedData.competitiveLandscape && (
                                <div className="glass-card p-6 md:p-10 border border-white/10 overflow-x-auto">
                                    ...
                                    <table className="w-full text-left border-collapse min-w-[800px]">
                                        <thead>
                                            <tr>
                                                <th className="font-mono text-xs uppercase tracking-widest text-white/40 pb-4 border-b border-white/10 w-1/5">Brands</th>
                                                <th className="font-mono text-xs uppercase tracking-widest text-white/40 pb-4 border-b border-white/10 w-1/5 text-center">Funding Status</th>
                                                <th className="font-mono text-xs uppercase tracking-widest text-white/40 pb-4 border-b border-white/10 w-1/5 text-center">Core Focus</th>
                                                <th className="font-mono text-xs uppercase tracking-widest text-white/40 pb-4 border-b border-white/10 w-1/5 text-center">Consumer Friction</th>
                                                <th className="font-mono text-xs uppercase tracking-widest text-white/40 pb-4 border-b border-white/10 w-1/5 text-center">Format & Storage</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            {parsedData.competitiveLandscape.map((comp, idx) => (
                                                <tr key={idx} className="group hover:bg-white/[0.02] transition-colors">
                                                    ...
                                                    <td className="py-4 border-b border-white/5 text-center">
                                                        <span className="inline-block px-3 py-1 bg-white/5 border border-white/10 rounded-lg text-xs font-mono text-white/60">
                                                            {comp.fundingStatus}
                                                        </span>
                                                    </td>
                                                    <td className="py-4 border-b border-white/5 text-center text-sm text-white/50">
                                                        {comp.coreFocus}
                                                    </td>
                                                    <td className="py-4 border-b border-white/5 text-center">
                                                    ...
                                                    <td className="py-4 border-b border-white/5 text-center font-mono text-xs text-white/50">
                        ...
                        <>
                            <div className="glass-card p-6 md:p-8">
                                ...
                                <p className="text-white/60 leading-relaxed text-lg">{parsedData.executiveSummary}</p>
                            </div>
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                <div className="glass-card p-6"><h3 className="text-xl font-bold mb-4">Insights</h3><ul className="space-y-4">{parsedData.marketInsights?.map((item, i) => <li key={i} className="text-sm text-white/60">{item}</li>)}</ul></div>
                                <div className="glass-card p-6"><h3 className="text-xl font-bold mb-4">Trends</h3><ul className="space-y-4">{parsedData.industryTrends?.map((item, i) => <li key={i} className="text-sm text-white/60">{item}</li>)}</ul></div>
                                <div className="glass-card p-6"><h3 className="text-xl font-bold mb-4">Opportunities</h3><ul className="space-y-4">{parsedData.keyOpportunities?.map((item, i) => <li key={i} className="text-sm text-white/60">{item}</li>)}</ul></div>
                            </div>
                        </>
                    )}
                </div>
